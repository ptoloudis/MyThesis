...
function SITL_connector(state,init_function,physics_function,max_timestep)
    ...
    target_rate = 120; % Target frequency in Hz
    target_period = 1 / target_rate; % Target time per iteration (4.167 ms)
...
% Init the UDP port
u = pnet('udpsocket',state.port);
disp (state.port);
unityport = state.port + 1;
...
while true
    ...
    % Enforce the 120  Hz rate
    elapsed_time = toc(iteration_start_time);
    % Check if enough time has passed to meet the target period
    if elapsed_time >= target_period
        % Initialize RPM array
        rpm_array = zeros(1, numel(state.copter.motors));
        for i = 1:numel(state.copter.motors)
            rpm_array(i) = state.copter.motors(i).rpm; % Store RPM
        end

        % Build JSON2 structure
        JSON2.rpms = rpm_array;
        JSON2.position = state.position;
        JSON2.attitude = state.attitude;

        %disp(state.position);

        % Send JSON2 data to the UDP socket
        pnet(u, 'printf', sprintf('\n%s\n', jsonencode(JSON2)));
        pnet(u, 'writepacket', 'localhost', unityport);
