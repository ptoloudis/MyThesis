...
% locations (xyz) in m
motor(1).location = [0.2,0.13,0]; % front right
motor(2).location = [0.2,-0.13,0]; % rear right
motor(3).location = [-0.2,-0.13,0]; % rear left
motor(4).location = [-0.2,0.13,0]; % front left
...
% motor properties
electrical.kv = 850; % (rpm/volt)
electrical.no_load_current = [0.6,20]; % (A) @ (V)
electrical.resistance = 0.272; % (ohms)

% ESC properties
esc.resistance = 0.01; % (ohms)

% Propeller properties
prop.diameter = 254 * 0.001; % (m)
prop.pitch = 114.3 * 0.001; % (m)
prop.num_blades = 2;
prop.PConst = 1.20;
prop.TConst = 1;
prop.mass = 0.02; % (kg) (only used for inertia)
...
% Setup battery
battery.voltage = 3*4.2; % (volts)
battery.resistance = 0.00317; % (ohms)
battery.capacity = 3.5; % (ah)

% Add all to vehicle
copter.motors = motor;
copter.battery = battery;
copter.mass = 1.282; % (kg)
...
jsonStr = jsonencode(copter);

% Write JSON string to a file
fileID = fopen('Copter.json', 'w');
if fileID == -1
